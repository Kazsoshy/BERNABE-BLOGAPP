...
<div class="container py-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="fw-bold text-dark">My <span class="text-primary">Blog</span></h1>
        <a href="{{ route('posts.create') }}" class="btn btn-primary rounded-pill px-4 d-flex align-items-center">
            ...
            <div class="card h-100 border-0 shadow-sm hover-shadow rounded-3 overflow-hidden">
                <div class="card-body">
                    <div class="d-flex justify-content-between mb-3">
                        <span class="badge bg-light text-dark rounded-pill px-3 py-2">#{{ $post->id }}</span>
                        <small class="text-muted">{{ $post->created_at->diffForHumans() }}</small>
                    </div>
                    <h5 class="card-title fw-bold mb-3">{{ $post->title }}</h5>
                    <p class="card-text text-muted">{{ \Illuminate\Support\Str::limit($post->body, 100) }}</p>
                </div>
    ...
    <div class="text-center py-5">
        ...
        <a href="{{ route('posts.create') }}" class="btn btn-primary rounded-pill px-4">Create New Post</a>
    </div>
    @endif
